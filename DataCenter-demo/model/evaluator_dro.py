...
class Evaluator:
    @staticmethod
    def oracle_evaluation(solution: dict, p, dataset: str):
        I, J, T = p.I, p.J, p.T
        cl, co = p.cl, p.co
        if dataset == "azur":
            d_test = p.d_hat_azur  # (S_test, J, T)
        else:
            d_test = p.d_hat_test
        ...
        for s in range(S_test):
            ...
            # 目标函数：最小化当前场景下的运营成本
            oper_cost = gp.quicksum(cl * l[j, t] for j in range(J) for t in range(T - 1)) + gp.quicksum(co * o[j] for j in range(J))
    ...
    @staticmethod
    def decision_rule_evaluation(solution: dict, p, dataset: str):
        I, J, T = p.I, p.J, p.T
        cl, co = p.cl, p.co
        if dataset == "azur":
            d_test = p.d_hat_azur  # (S_test, J, T)
                ...
                for i in range(I):
                    for j in range(J):
                        ...
                        if t == 0:
                            ...
                            phi = [d_test[s, jp, t - 1] for jp in range(J)]
